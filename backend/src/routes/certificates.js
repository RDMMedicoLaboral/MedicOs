...
// POST /api/certificates -> crea el certificado (toma snapshot del paciente y del médico)
certificatesRouter.post("/", (req, res) => {
  ...
  const patient = db.prepare(`SELECT * FROM patients WHERE id = ? AND clinic_id = ?`).get(patient_id, req.user.clinic_id);
  ...
  logAudit({ clinicId: req.user.clinic_id, actor: req.user.username, action: "create", entity: "certificate", entityId: result.lastInsertRowid });

  const certificate = db.prepare(`SELECT * FROM certificates WHERE id = ?`).get(result.lastInsertRowid);
  res.status(201).json(certificate);
});

// GET /api/certificates/patient/:patientId -> historial de certificados del paciente
certificatesRouter.get("/patient/:patientId", (req, res) => {
  const rows = db
...
certificatesRouter.get("/:id/pdf", (req, res) => {
  ...
  row("Código CIE-10/11:", cert.diagnosis_code);
